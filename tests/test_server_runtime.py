...
def test_apply_runtime_provider_defaults_autofills_official_openai_lanes():
    ...
    assert normalized["OUROBOROS_MODEL"] == "openai::gpt-5.2"
    assert normalized["OUROBOROS_MODEL_CODE"] == "openai::gpt-5.2"
    assert normalized["OUROBOROS_MODEL_LIGHT"] == "openai::gpt-4.1"
    assert normalized["OUROBOROS_MODEL_FALLBACK"] == "openai::gpt-4.1"
    assert normalized["OUROBOROS_REVIEW_MODELS"] == ",".join(["openai::gpt-5.2"] * 3)
...
def test_apply_runtime_provider_defaults_keeps_explicit_official_openai_review_models():
    normalized, changed, changed_keys = apply_runtime_provider_defaults({
        ...
        "OUROBOROS_MODEL_LIGHT": "openai::gpt-4.1",
        "OUROBOROS_MODEL_FALLBACK": "openai::gpt-4.1",
        "OUROBOROS_REVIEW_MODELS": "openai::gpt-5.2,openai::gpt-4.1",
    })

    assert not changed
    assert changed_keys == []
    assert normalized["OUROBOROS_REVIEW_MODELS"] == "openai::gpt-5.2,openai::gpt-4.1"
